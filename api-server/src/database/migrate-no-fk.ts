...
const createTablesNoFK = async () => {
  ...
  try {
    ...
    console.log('✅ Database tables created successfully (without foreign keys)')
...
if (import.meta.url === `file://${process.argv[1]}`) {
  createTablesNoFK()
    .then(() => {
      console.log('🎉 Database migration completed successfully (no foreign keys)')
      process.exit(0)
    })
    .catch((error) => {
      console.error('💥 Database migration failed:', error)
